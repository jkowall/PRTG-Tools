...
try:
    from impacket.dcerpc.v5 import dcom, wmi
...
class NetworkScanner:
        ...
        for cidr in self.cidrs:
            ...
            try:
                ...
                with concurrent.futures.ThreadPoolExecutor(max_workers=50) as executor:
                    future_to_ip = {
                        executor.submit(self._check_host, str(ip), arp_table.get(str(ip))): str(ip) 
                        for ip in hosts
                    }
    ...
    def _probe_snmp(self, ip, info):
        ...
        try:
            ...
            if not errorIndication and not errorStatus:
                ...
                logger.info(f"SNMP Identification for {ip}: {info['vendor']} - {info['os']}")
    ...
    def _probe_ssh(self, ip, info):
        ...
        try:
            ...
            stdin, stdout, stderr = client.exec_command("cat /etc/os-release; uname -r", timeout=3)
    ...
    def _probe_wmi(self, ip, info):
        ...
        try:
            ...
            dcomConnection = DCOMConnection(ip, user, password, domain, 
                                          object=wmi.CLSID_WbemLevel1Login, 
                                          oxidResolver=True)
            iInterface = dcomConnection.CoCreateInstanceEx(wmi.CLSID_WbemLevel1Login, wmi.IID_IWbemLevel1Login)
    ...
    def _get_mac_wsl(self, ip):
        ...
        try:
            ...
            cmd = ["arp.exe", "-a"]
            # specific ip argument for arp.exe works on windows? "arp -a 192.168.1.1" -> "No ARP Entries Found" if not specific.
...
class Reporter:
    ...
    def generate_csv(self, data, filename="Hybrid_Assessment_Report.csv"):
        ...
        keys = ["IP Address", "Hostname", "Source", "MAC Address", "Hardware Vendor", 
                "Hardware Model", "OS Version", "Monitoring Status", 
