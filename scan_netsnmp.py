...
try:
    ...
    print("Error: pysnmp library is required.")
...
def query_snmp(ip: str, community: str, timeout: float, retries: int) -> Tuple[str, Optional[str], Optional[str]]:
    """
...
def scan_subnet(
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for i, future in enumerate(as_completed(futures), 1):
            ...
            if i % 10 == 0 or i == total_hosts:
                print(f"\r  Progress: {i}/{total_hosts} hosts scanned", end="", flush=True)
    ...
    print(f"  {Colors.BLUE}Responding: {responding_count} | Vulnerable: {vulnerable_count}{Colors.END}")
...
def print_results(results: List[dict], verbose: bool):
    ...
    if vulnerable:
        ...
        print(f"║  VULNERABLE DEVICES FOUND - CVE-2025-68615                       ║")
        print(f"╚══════════════════════════════════════════════════════════════════╝{Colors.END}")
        print(f"\n{Colors.RED}The following devices are running Net-SNMP < 5.9.5 and should be")
        print(f"patched immediately to address the snmptrapd vulnerability:{Colors.END}\n")
        ...
        for r in sorted(vulnerable, key=lambda x: ipaddress.ip_address(x["ip"])):
            ...
            sys_descr_short = r["sys_descr"][:45] + "..." if len(r["sys_descr"]) > 48 else r["sys_descr"]
            print(f"{Colors.RED}{r['ip']:<18} {version_display:<12} {sys_descr_short}{Colors.END}")
    ...
    if verbose and safe_netsnmp:
        ...
        for r in sorted(safe_netsnmp, key=lambda x: ipaddress.ip_address(x["ip"])):
            version_display = r["version"] or "Unknown"
            sys_descr_short = r["sys_descr"][:45] + "..." if len(r["sys_descr"]) > 48 else r["sys_descr"]
            print(f"{Colors.GREEN}{r['ip']:<18} {version_display:<12} {sys_descr_short}{Colors.END}")
    ...
    if verbose and other:
        ...
        for r in sorted(other, key=lambda x: ipaddress.ip_address(x["ip"])):
            sys_descr_short = r["sys_descr"][:60] + "..." if len(r["sys_descr"]) > 63 else r["sys_descr"]
